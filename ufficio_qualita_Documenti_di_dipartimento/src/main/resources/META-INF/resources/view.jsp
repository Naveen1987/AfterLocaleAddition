...
<table class="table">
  <thead>
   ...
   <th style="width:.5px"></th>
    ...
    <th class="otherhead"><b>Dipartimento</b></th>
    <th class="widthData"><b>TITOLO</b></th>
   <th class="otherhead"><b>REV</b></th>
    <th class="otherhead"><b>Dimensioni</b></th>
    <th class="otherhead"><b>LINGUA</b></th>
    <th style="width:2px"></th>
    </tr>
    <tr class="warning no-result">
      <td colspan="9"><i class="fa fa-warning"></i> <liferay-ui:message key="noresult"/></td>
    </tr>
  </thead>
  <tbody>
  <tr>
...
<%
	...
	String cod="",titolo="",lingua="",rev="",dipartimento="";
	FileServiceFileLib fsfl=new FileServiceFileLib();
	List<FileLibInfo> records=fsfl.getRecords("Documenti di dipartimento", themeDisplay,locale);
	for(FileLibInfo record:records){
		for(Map.Entry<String,String> entry:record.getFields().entrySet()){
			...
			else if(entry.getKey().equalsIgnoreCase("dipartimento")){
				dipartimento=entry.getValue().toString();
			}
		}
%>
<tr>
<td  class="otherdata"><aui:a href="<%=record.getDownloadUrl()%>" style="color:black;text-decoration:none;"><%=getValue(cod)%></aui:a></td>
<td  class="otherdata"><aui:a href="<%=record.getDownloadUrl()%>" style="color:black;text-decoration:none;"><%=getValue(dipartimento)%></aui:a></td>
<td class="widthData"><aui:a href="<%=record.getDownloadUrl()%>" style="color:black;text-decoration:none;"><%=getValue(titolo)%></aui:a></td> 
<td  class="otherdata"><aui:a href="<%=record.getDownloadUrl()%>" style="color:black;text-decoration:none;"><%=getValue(rev)%></aui:a></td>
<td  class="otherdata">
...
<td><aui:a href="<%=record.getDownloadUrl()%>" style="color:black;text-decoration:none;"><%=dipartimento%></aui:a></td>
